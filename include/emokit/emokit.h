...
extern "C"
{
	...
	struct emokit_contact_quality {//values > 4000 are good
		char counter; //loops from 0 to 128 (129 values)
	 ...
	 * @return <0 for error, otherwise return the number of bytes read.
	 */
	EMOKIT_DECLSPEC int emokit_read_data_timeout(struct emokit_device* dev, unsigned timeout);

	EMOKIT_DECLSPEC int emokit_get_next_frame(struct emokit_device* dev,struct emokit_frame* k);


	EMOKIT_DECLSPEC int emokit_get_qnext_frame(struct emokit_device* s, struct emokit_contact_quality* k);
