...
EMOKIT_DECLSPEC
int emokit_get_next_frame(struct emokit_device* s, struct emokit_frame* k) {
...
EMOKIT_DECLSPEC
int emokit_get_qnext_frame(struct emokit_device* s, struct emokit_contact_quality* k) {
	struct emokit_contact_quality q;
	memset(s->raw_unenc_frame, 0, 32);
	...
	memset(&q,0,sizeof(struct emokit_contact_quality));
	if (s->raw_unenc_frame[0] & 128) {
		k->counter = 128;
	} else {
		k->counter = s->raw_unenc_frame[0];
